...
function getAccessibilityLevel(ratio) {
	...
	if (ratio >= 3)
		return { level: 'AA Large', status: '⚠️', description: 'Apenas para texto grande' };
	...
	['light', 'dark'].forEach((theme) => {
		...
		testCombinations.forEach(([bg, text]) => {
			if (palette[theme][bg] && palette[theme][text]) {
				...
				console.log(
					`│ ${combinationName} │ ${ratioStr} │ ${levelStr} │ ${statusStr} │ ${descStr} │`
				);
			}
		});
...
if (
	...
	process.argv[1].endsWith('accessibility-checker.js')
) {
