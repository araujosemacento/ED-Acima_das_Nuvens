...
function createThemeStore() {
	...
	const currentTheme = derived(
		...
		([$userTheme, $systemTheme, $hasUserInteraction]) => {
			...
			if (
				$hasUserInteraction &&
				($userTheme === THEME_TYPES.LIGHT || $userTheme === THEME_TYPES.DARK)
			) {
	...
	const applyTheme = (theme) => {
		...
		const existingClasses = Array.from(document.documentElement.classList).filter((cls) =>
		...
		const animate = () => {
			...
			// Interpolar cores
			const newBg = interpolateHSL(
				currentBg || 'hsl(145, 100%, 95%)',
				targetColors.background,
				easedProgress
			);
			const newSurface = interpolateHSL(
				currentSurface || 'hsl(171, 28%, 90%)',
				targetColors.surface,
				easedProgress
			);
			const newText = interpolateHSL(
				currentText || 'hsl(162, 100%, 10%)',
				targetColors.text,
				easedProgress
			);
